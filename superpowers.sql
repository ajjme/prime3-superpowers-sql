...
SELECT heroes.name, heroes.alias, powers.name, powers.description
FROM
	heroes
...
INSERT INTO powers(name, description) VALUES('Super Public Speaking','Can speak in front of audiences of twenty or more');

-- 5. Add this power to an existing hero.
INSERT INTO heroes_powers(hero_id, power_id, power_level) VALUES(4, 9, 1);
...
SELECT count(*) AS "can_fly"
FROM heroes_powers
WHERE heroes_powers.power_id = 3;

-- 8. What is the average super strength power level?
SELECT avg(heroes_powers.power_level) AS "average_strength_level"
FROM heroes_powers
WHERE power_id = 4;
...
SELECT powers.name, powers.description, avg(heroes_powers.power_level) AS "average_power_level"
FROM
	powers
	JOIN heroes_powers ON powers.id = heroes_powers.power_id
GROUP BY (powers.name, powers.description)
ORDER BY average_power_level DESC;
